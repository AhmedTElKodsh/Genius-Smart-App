...
const SendRequestModal: React.FC<SendRequestModalProps> = ({
  ...
  React.useEffect(() => {
    if (isOpen && teacherData?.id) {
      const fetchRemainingHours = async () => {
        try {
          const response = await fetch(`${API_BASE_URL}/teachers/${teacherData.id}/remaining-hours`);
  ...
  const countMonthlyPermittedRequests = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/requests/teacher/${teacherData.id}/all`);
      const data = await response.json();
