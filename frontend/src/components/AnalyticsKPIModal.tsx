...
const TableTitle = styled.h4<{ isDarkMode: boolean; isRTL?: boolean }>`
...
const AnalyticsKPIModal: React.FC<AnalyticsKPIModalProps> = ({
  ...
  const translateDepartment = (dept: string) => {
    ...
    if (dept === 'Floor Admin') return t('subjects.floorAdmin');
    if (dept === 'KG Manager') return t('subjects.kgManager');
    
    // For other departments, use dynamic key
    const deptKey = `subjects.${dept.toLowerCase().replace(/\s+/g, '')}`;
    return t(deptKey) || dept;
  };

  const translateWorkType = (workType: string) => {
    if (workType === 'Full-time') return t('reports.fullTime') || 'Full-time';
    if (workType === 'Part-time') return t('reports.partTime') || 'Part-time';
    return workType;
  };
      ...
      case 'totalTeachers':
        return (
          <>
            ...
            <TableCell isDarkMode={isDarkMode} isRTL={isRTL}>{translateDepartment(teacher.department)}</TableCell>
            <TableCell isDarkMode={isDarkMode} isRTL={isRTL}>{translateWorkType(teacher.performance || 'Full-time')}</TableCell>
      ...
      case 'attendanceRate':
        return (
          <>
            ...
            <TableCell isDarkMode={isDarkMode} isRTL={isRTL}>{teacher.absences || 0} {t('analytics.days') || 'days'}</TableCell>
            <TableCell isDarkMode={isDarkMode} isRTL={isRTL}>{teacher.workHours || 0}{t('analytics.hours') || 'h'}</TableCell>
          </>
        );
      case 'topPerformers':
        return (
      ...
      case 'departments':
        return (
          <>
            ...
            <TableCell isDarkMode={isDarkMode} isRTL={isRTL}>{translateDepartment(teacher.department)}</TableCell>
            <TableCell isDarkMode={isDarkMode} isRTL={isRTL}>{teacher.attendance || 0}%</TableCell>
            <TableCell isDarkMode={isDarkMode} isRTL={isRTL}>{translateRole(teacher.performance || 'Teacher')}</TableCell>
            <TableCell isDarkMode={isDarkMode} isRTL={isRTL}>
              <StatusBadge $status={teacher.status || 'good'}>
