...
const TeacherHomeAdvanced: React.FC = () => {
  ...
  useEffect(() => {
    const checkCurrentStatus = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/attendance?teacherId=${teacherData.id}&startDate=${today}&endDate=${today}`);
  ...
  useEffect(() => {
    const fetchLatestNotification = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/teachers/${teacherData.id}/notifications`);
        if (response.ok) {
          const data = await response.json();
  ...
  const fetchAttendanceSummary = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/attendance/summary/${teacherData.id}?${queryParams}`);
      const data = await response.json();
      ...
      const balanceResponse = await fetch(`${API_BASE_URL}/teachers/${teacherData.id}/balance`);
  ...
  const handleLocationVerified = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/attendance/checkin/${teacherData.id}`, {
        method: 'POST',
        headers: {
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      ...
      const endDateStr = endDate.toISOString().split('T')[0];
             ...
             const response = await fetch(`${API_BASE_URL}/attendance/teacher/${teacherData.id}?startDate=${startDateStr}&endDate=${endDateStr}&lang=${language}`);
       if (response.ok) {
         const data = await response.json();
