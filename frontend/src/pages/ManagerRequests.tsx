...
const Requests: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        if (isAdmin) {
          const nonAdminResponse = await fetch(`${API_BASE_URL}/requests/non-admin-handled`, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
          ...
          const allHandledResponse = await fetch(`${API_BASE_URL}/requests/all-handled`, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
            }
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${API_BASE_URL}/requests/${requestToConfirm.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
