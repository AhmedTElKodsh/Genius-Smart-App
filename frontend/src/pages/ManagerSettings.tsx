...
const Settings: React.FC = () => {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/manager/change-password`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleTestNotification = async (requestType: string) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/manager/test-notification`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const saveHoliday = async (holidayData: any) => {
    try {
      ...
      const url = editingHoliday 
        ? `${API_BASE_URL}/holidays/${editingHoliday.id}`
