...
const RequestTypeGrid = styled.div<{ isRTL: boolean; hasError?: boolean }>`
  display: grid;
...
const DateInput = styled.input<{ isDarkMode: boolean; isRTL: boolean; hasError?: boolean }>`
  width: 100%;
  padding: 15px;
  ...
  font-size: 16px;
  background: ${props => {
    if (props.hasError) return props.isDarkMode ? 'rgba(255, 68, 68, 0.1)' : 'rgba(255, 68, 68, 0.05)';
  ...
  &:focus {
    outline: none;
    border-color: ${props => props.hasError ? '#ff4444' : '#DAA520'};
    box-shadow: 0 0 0 3px ${props => props.hasError ? 'rgba(255, 68, 68, 0.2)' : 'rgba(218, 165, 32, 0.1)'};
  }
...
const HourSelectionSection = styled.div<{ isRTL: boolean; hasError?: boolean }>`
  margin-bottom: 20px;
  direction: ${props => props.isRTL ? 'rtl' : 'ltr'};
  ${props => props.hasError && `
    padding: 3px;
...
const SendRequestModal: React.FC<SendRequestModalProps> = ({
  ...
  // Field validation states for immediate feedback
  const [showTypeError, setShowTypeError] = useState(false);
  const [showDateError, setShowDateError] = useState(false);
  const [showHourSelectionError, setShowHourSelectionError] = useState(false);
  ...
  const [totalHours, setTotalHours] = useState<number>(4); // Default 4 hours total
  ...
  const handleTypeSelection = (type: string) => {
    ...
    // Clear validation errors when type is selected
    setShowTypeError(false);
    setShowHourSelectionError(false);
  };
  ...
  // Validation helper functions for immediate feedback
  ...
  const validateHourSelection = () => {
    if (selectedType === 'lateArrival' || selectedType === 'earlyLeave') {
      const hasValidHours = useCustomHours 
        ? customHours && parseFloat(customHours) > 0
        : selectedHours > 0;
      
      if (!hasValidHours) {
        setShowHourSelectionError(true);
        return false;
      }
    }
    setShowHourSelectionError(false);
    ...
    <ModalOverlay isOpen={isOpen} onClick={handleClose}>
      ...
      >
        ...
        <Content>
          ...
          <RequestTypeGrid isRTL={isRTL} hasError={showTypeError}>
            {requestTypes.map((type) => (
              <RequestTypeCard
                ...
                onBlur={() => {
                  // Validate when user moves focus away from request type section
                  setTimeout(() => validateRequestType(), 100);
                }}
              >
                {type.label}
              </RequestTypeCard>
          ...
          {(selectedType === 'lateArrival' || selectedType === 'earlyLeave') && (
            <HourSelectionSection isRTL={isRTL} hasError={showHourSelectionError}>
              ...
              <RemainingHoursInfo isDarkMode={isDarkMode} isRTL={isRTL}>
                {(() => {
                  ...
                  if (selectedHoursValue > 0) {
                    ...
                    return isRTL 
                      ? `الساعات المتبقية: ${previewRemainingHours} من ${totalHours} ساعات (بعد هذا الطلب)`
                      : `Remaining Hours: ${previewRemainingHours} out of ${totalHours} hours (after this request)`;
                  } else {
                    return isRTL 
                      ? `الساعات المتبقية: ${remainingHours} من ${totalHours} ساعات`
                      : `Remaining Hours: ${remainingHours} out of ${totalHours} hours`;
                  }
                })()}
              </RemainingHoursInfo>
              ...
              <HourOptionsGrid 
                isRTL={isRTL}
                onBlur={() => {
                  // Validate when user moves focus away from hour selection
                  setTimeout(() => validateHourSelection(), 100);
                }}
              >
                <HourOption
                  isDarkMode={isDarkMode}
                  ...
                  onClick={() => {
                    ...
                    setCustomHours('');
                    setShowHourError(false);
                    setShowHourSelectionError(false); // Clear error when selected
                  }}
                >
              ...
              {useCustomHours && (
                <CustomHourInput
                  ...
                  onChange={(e) => {
                    ...
                    setShowHourError(false);
                    setShowHourSelectionError(false); // Clear error when typing
                    // The preview will automatically update due to the dynamic calculation above
                  }}
                  onBlur={() => validateHourSelection()}
