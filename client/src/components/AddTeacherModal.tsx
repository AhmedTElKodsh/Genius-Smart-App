...
const AddTeacherModal: React.FC<AddTeacherModalProps> = ({ isOpen, onClose, onSuccess, managerAuthorities }) => {
  ...
  const currentUserRole = managerInfo ? JSON.parse(managerInfo).role : null;
  ...
  // Handle role change and update authorities
  const handleRoleChange = (role: string) => {
    ...
    switch (role) {
      ...
      case 'EMPLOYEE':
      default:
        // Default authorities for employee (all false)
        break;
    }
      ...
      <ModalContent>
        ...
        <ModalBody>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <FormFieldFull>
              <Label>{translations?.['form.employmentDate'] || 'Employment Date *'}</Label>
              <Input
                type="date"
              ...
              />
              <HelperText>
                {translations?.['addTeacher.employmentDateHelper'] || 'When did the teacher join your institution?'}
              </HelperText>
            </FormFieldFull>

            <FormFieldFull>
              <Label>{translations?.['form.allowedAbsenceDays'] || 'Allowed Absence Days (per year) *'}</Label>
              <Input
                type="number"
                min="0"
                max="365"
              ...
              />
              <HelperText>
                {translations?.['addTeacher.allowedAbsenceDaysHelper'] || 'Maximum number of days this teacher can be absent per year'}
              </HelperText>
            </FormFieldFull>
            ...
            {managerAuthorities?.canManageAuthorities && (
              <AuthoritySection>
                <AuthorityTitle>
                  {translations?.['authorities.title'] || 'Authorities'}
                </AuthorityTitle>
                <AuthorityGrid>
                  <AuthorityItem>
                    <AuthorityCheckbox
            ...
            {currentUserRole === 'ADMIN' && (
              <FormGrid>
                <FormFieldFull>
                  <Label>{translations?.['addTeacher.systemRole'] || 'System Role'}</Label>
                  <Select
                    value={formData.systemRole}
                    onChange={(e) => handleRoleChange(e.target.value)}
                  >
                    <option value="ADMIN">
                      {translations?.['systemRoles.admin'] || 'Admin'} - {translations?.['systemRoles.adminDescription'] || 'Manage all users, subjects, and system settings.'}
                    </option>
                    <option value="MANAGER">
                      {translations?.['systemRoles.manager'] || 'Manager'} - {translations?.['systemRoles.managerDescription'] || 'Manage teachers, students, and handle requests.'}
                    </option>
                    <option value="EMPLOYEE">
                      {translations?.['systemRoles.employee'] || 'Employee'} - {translations?.['systemRoles.employeeDescription'] || 'Perform basic tasks and access limited features.'}
                    </option>
                  </Select>
                </FormFieldFull>
              </FormGrid>
