...
const TeacherNotifications: React.FC = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const requestResponse = await fetch(`http://localhost:5000/api/requests/teacher/${teacherData.id}${langParam}`);
      const requestData = await requestResponse.json();

      // Fetch attendance notifications  
      const attendanceResponse = await fetch(`http://localhost:5000/api/attendance/notifications/${teacherData.id}${langParam}`);
      const attendanceData = await attendanceResponse.json();

      // Fetch general teacher notifications (including request approvals/rejections)
      const teacherNotificationsResponse = await fetch(`http://localhost:5000/api/teachers/${teacherData.id}/notifications`);
      const teacherNotificationsData = await teacherNotificationsResponse.json();

      // Combine and group notifications
      const allNotifications = [
        ...(requestData.success ? requestData.data : []),
        ...(attendanceData.success ? attendanceData.data : []),
        ...(teacherNotificationsData.success ? teacherNotificationsData.data : [])
      ];
