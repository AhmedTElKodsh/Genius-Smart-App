...
interface SummaryData {
  ...
  overtime: number;
  lateArrival: number;
  remainingAbsenceDays?: number;
  remainingLateEarlyHours?: number;
  usedAbsenceDays?: number;
  usedLateEarlyHours?: number;
}
...
const TeacherTimerPage: React.FC<TeacherTimerPageProps> = () => {
  ...
  const [summaryData, setSummaryData] = useState<SummaryData>({
    ...
    remainingLateEarlyHours: 0,
    usedAbsenceDays: 0,
    usedLateEarlyHours: 0
  });
  ...
  const fetchAttendanceSummary = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        // Also fetch teacher data to get remaining hours/days
        const teacherResponse = await fetch(`http://localhost:5000/api/teachers/${teacherData.id}`);
        const teacherInfo = await teacherResponse.json();
        
        setSummaryData({
          ...data.data,
          remainingAbsenceDays: teacherInfo.data?.remainingAbsenceDays || teacherInfo.data?.allowedAbsenceDays || 12,
          remainingLateEarlyHours: teacherInfo.data?.remainingLateEarlyHours || 4,
          usedAbsenceDays: teacherInfo.data?.usedAbsenceDays || 0,
          usedLateEarlyHours: teacherInfo.data?.usedLateEarlyHours || 0
        });
      } else {
        ...
        setSummaryData({
          ...
          overtime: 0,
          lateArrival: 0,
          remainingAbsenceDays: teacherData?.remainingAbsenceDays || teacherData?.allowedAbsenceDays || 12,
          remainingLateEarlyHours: teacherData?.remainingLateEarlyHours || 4,
          usedAbsenceDays: 0,
          usedLateEarlyHours: 0
        });
      }
            ...
            >
              <SummaryLabel isDarkMode={isDarkMode} isRTL={isRTL}>
                ...
                <SummaryItem
                  isDarkMode={isDarkMode}
                  isRTL={isRTL}
                  onClick={() => handleSummaryCardClick('remainingAbsenceDays')}
                >
                  <SummaryLabel isDarkMode={isDarkMode} isRTL={isRTL}>
                    {isRTL ? 'أيام الغياب المتبقية' : 'Remaining Absence Days'}
                  </SummaryLabel>
                  <SummaryValue isDarkMode={isDarkMode} isRTL={isRTL}>
                    {loading ? '⏳' : `${summaryData.remainingAbsenceDays?.toString().padStart(2, '0') || '00'} ${t.days}`}
                  </SummaryValue>
                </SummaryItem>
                <SummaryItem
                  isDarkMode={isDarkMode}
                  isRTL={isRTL}
                  onClick={() => handleSummaryCardClick('remainingLateEarlyHours')}
                >
                  <SummaryLabel isDarkMode={isDarkMode} isRTL={isRTL}>
                    {isRTL ? 'ساعات التأخير/المغادرة المتبقية' : 'Remaining Late/Early Hours'}
                  </SummaryLabel>
                  <SummaryValue isDarkMode={isDarkMode} isRTL={isRTL}>
                    {loading ? '⏳' : `${summaryData.remainingLateEarlyHours?.toString().padStart(2, '0') || '00'} ${isRTL ? 'ساعات' : 'hours'}`}
                  </SummaryValue>
                </SummaryItem>
              </SummaryGrid>
            </SummarySection>
